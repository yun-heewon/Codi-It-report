...
---
## 8. 프로젝트에 대한 셀프 평가 및 느낌점
- 마지막 프로젝트이나 다른팀 처럼 새로운 프레임워크나 ORM을 써보지 못한 것이 아쉬움. 모든 기능 구현 이후 새로운 추가 기능을 개발하거나, 새로운 라이브러리를 사용해보기로 하였으나 프로젝트를 완성하지 못해 진행하지 못하게 되어 아쉬움.
- 소통에서의 적극성이 부족했다고 느낌. 이전 프로젝트에서는 소통을 이끌어가며 진행한 면모가 많았으나, 이번 프로젝트에서는 그로 인한 피로감과 전반적인 팀의 분위기가 소통에 소극적이어서 이를 따라가게 됨. 결론적으로는 프로젝트를 완성시키지 못하게 된 큰 이유 중 하나라고 생각됨.
- 코드 작성 능력이 아직 부족하다고 느껴짐. 클래스를 여러번 작성하며 코드에 많이 익숙해졌지만 지금도 혼자 코드를 작성하라고 한다면 어려울 것 같고, 옆에 참고할 비슷한 코드가 있어야 코드 작성이 가능함. 그렇다보니 코드를 읽는 힘이 아직도 부족하다고 느껴지고, 이 외에도 아직 구현하기 어려운 기능들이 많다고 느껴짐.
- 내가 맡은 부분에 대해서는 전반적으로 잘 구현했다고 생각되나, 아직 공부해야할 코드들이 많고 코드에 대한 이해도를 더 높여야겠다고 생각됨 
